...
export class GameRenderer {
    ...
    constructor(canvas) {
        ...
        this.ctx = ctx;
        for (let i = 0; i < 220; i++)
            this.starSeed.push({ x: Math.random(), y: Math.random(), size: Math.random() * 1.5 + 0.4, alpha: Math.random() * 0.22 + 0.05 });
        for (let i = 0; i < 8; i++)
            this.nebulaSeed.push({ x: Math.random(), y: Math.random(), radius: 180 + Math.random() * 340, hue: [196, 225, 268, 302][i % 4] });
        this.resize();
    }
    setArenaRadius(radius) { this.arenaRadius = radius; }
    setPlayerId(id) { this.playerId = id; }
    setEffects(enabled) { this.effects = enabled; if (!enabled)
        this.particles = []; }
    resize() {
    ...
    handleEvent(event) {
        ...
        if (event.type === "collect") {
            ...
            if (event.eaterId === this.playerId)
                this.flash = Math.max(this.flash, .09 + Math.min(.12, event.value * .015));
            return;
        }
        if (event.type === "death") {
            const skin = skinFor(event.skin);
            ...
            if (distance < 900)
                this.shake = Math.max(this.shake, event.snakeId === this.playerId ? 12 : 5);
            if (event.snakeId === this.playerId)
                this.flash = .45;
        }
    }
    render(pair, now, dt, input, replaying) {
        ...
        if (pair) {
            ...
            for (const snake of snakes)
                this.drawSnake(ctx, snake, snake.id === this.playerId, now);
        ...
        if (replaying)
            this.drawReplayTreatment(ctx, width, height, now);
        if (this.flash > .001) {
    ...
    drawFood(ctx, foods, now) {
        ...
        for (const f of foods) {
            if (Math.abs(f.x - this.camera.x) > viewX || Math.abs(f.y - this.camera.y) > viewY)
                continue;
            ...
            if (this.effects && f.value >= 3) {
                ...
                ctx.beginPath();
                ctx.arc(f.x, f.y, r * 3.3 * pulse, 0, Math.PI * 2);
                ctx.fill();
            }
            ctx.fillStyle = `hsla(${hue},96%,68%,.23)`;
            ctx.beginPath();
            ctx.arc(f.x, f.y, r * 2.05 * pulse, 0, Math.PI * 2);
            ctx.fill();
            ...
            if (f.kind === 2) {
                ...
                ctx.beginPath();
                ctx.arc(f.x - r * .24, f.y - r * .24, Math.max(1, r * .28), 0, Math.PI * 2);
                ctx.fill();
            }
        }
    }
    drawSnake(ctx, s, isMe, now) {
        if (s.body.length < 2)
            return;
        const head = s.body[0];
        const viewX = this.canvas.clientWidth / this.camera.zoom * .7 + 520;
        const viewY = this.canvas.clientHeight / this.camera.zoom * .7 + 520;
        if (!isMe && (Math.abs(head.x - this.camera.x) > viewX || Math.abs(head.y - this.camera.y) > viewY))
            return;
        ...
        ctx.beginPath();
        ctx.arc(head.x, head.y, radius * 1.22 * headScale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(head.x, head.y, radius * 2.7, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const sign of [-1, 1]) {
            ...
            ctx.beginPath();
            ctx.arc(ex, ey, radius * .30, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = "#0c1020";
            ctx.beginPath();
            ctx.arc(ex + dx * radius * .1, ey + dy * radius * .1, radius * .13, 0, Math.PI * 2);
            ctx.fill();
        }
    ...
    drawPattern(ctx, s, palette, radius, now) {
        if (palette.pattern === "solid")
            return;
    ...
    drawParticles(ctx) {
        for (const p of this.particles) {
            ...
            ctx.beginPath();
            ctx.arc(p.x, p.y, Math.max(.5, p.size * (.35 + t * .65)), 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.globalAlpha = 1;
    }
    ...
    pushParticle(x, y, vx, vy, life, size, color, drag) {
        if (!this.effects)
            return;
        this.particles.push({ x, y, vx, vy, life, maxLife: life, size, color, drag });
    }
    drawMinimap(ctx, width, height, me) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.arc(x + size / 2, y + size / 2, size * .32, 0, Math.PI * 2);
        ctx.stroke();
        if (me) {
    ...
    drawBoundaryWarning(width, height, me) {
        if (!me)
            return;
    ...
    drawReplayTreatment(ctx, width, height, now) {
        const scan = (now * .08) % 8;
        ctx.save();
        ctx.globalAlpha = .055;
        ctx.fillStyle = "#b3e6ff";
        for (let y = scan; y < height; y += 8)
            ctx.fillRect(0, y, width, 1);
        ctx.restore();
    }
}
function interpolateSnakes(prev, next, t) {
    const prevMap = new Map(prev.map(s => [s.id, s]));
    return next.map(n => {
        const p = prevMap.get(n.id);
        if (!p)
            return n;
        const body = n.body.map((point, i) => {

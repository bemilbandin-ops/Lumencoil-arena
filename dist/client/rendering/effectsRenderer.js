...
export class EffectsRenderer {
    ...
    handleEvent(event, playerId, camera) {
        if (event.type === "spawn") {
        ...
        if (event.type === "death") {
            ...
            if (Math.hypot(event.x - camera.x, event.y - camera.y) < 900)
                this.shake = Math.max(this.shake, event.snakeId === playerId ? 12 : 5);
            if (event.snakeId === playerId)
                this.flash = .45;
        }
    }
    spawnTForSnake(snakeId, now) {
        const spawnAt = this.spawnFx.get(snakeId);
        const spawnT = spawnAt ? clamp((now - spawnAt) / 520, 0, 1) : 1;
        if (spawnAt && spawnT >= 1)
            this.spawnFx.delete(snakeId);
        return spawnT;
    }
    getShakeOffset() {
        if (!this.effects || this.shake <= .1)
            return { x: 0, y: 0 };
        return { x: rand(-this.shake, this.shake), y: rand(-this.shake, this.shake) };
    }
    emitBoostTrail(s) {
        if (!this.effects || !s.boost || s.body.length < 2)
            return;
        const head = s.body[0];
    ...
    update(dt) { this.updateParticles(dt); }
    drawFrameOverlays(ctx, width, height, now, dt, replaying) {
        if (replaying)
...
//# sourceMappingURL=effectsRenderer.js.map
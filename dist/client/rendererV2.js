...
import { drawBackdrop, drawBoundaryWarning, drawFood, drawJoystick, drawMinimap, drawSnake, drawWorldBoundary } from "./rendererVisuals.js";
export class GameRenderer {
    ...
    handleEvent(event) {
        ...
        if (event.type === "death") {
            const skin = skinFor(event.skin);
            if (this.effects)
                for (let i = 0, count = Math.min(68, 26 + Math.floor(event.mass * .22)); i < count; i++) {
            ...
            if (Math.hypot(event.x - this.camera.x, event.y - this.camera.y) < 900)
    ...
    render(pair, now, dt, input, replaying) {
        ...
        const env = { canvas: this.canvas, ctx, camera: this.camera, arenaRadius: this.arenaRadius, effects: this.effects };
        ...
        if (input.joystickAnchor)
            drawJoystick(ctx, input.joystickAnchor, input.joystickKnob ?? input.joystickAnchor);
        if (replaying)

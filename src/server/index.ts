...
class WsPeer {
  ...
  private onData(chunk: any): void {
    ...
    while (this.buffer.length >= 2 && !this.closed) {
      ...
      if (rsv !== 0 || !masked) return this.close(1002, "Protocol error");
      ...
      if (len === 126) {
        ...
        if (big > BigInt(MAX_WS_PAYLOAD)) return this.close(1009, "Message too large");
        len = Number(big);
        offset = 10;
      }
      const control = opcode >= 0x8;
      if (control && (!fin || len > 125)) return this.close(1002, "Invalid control frame");
      if (len > MAX_WS_PAYLOAD) return this.close(1009, "Message too large");
      ...
      if (opcode === 0x2) { this.close(1003, "Binary unsupported"); return; }

      if (opcode === 0x1) {
        if (this.fragmentOpcode !== null) { this.close(1002, "Unexpected text frame"); return; }
        if (fin) this.consumeText(payload);
        else {
      ...
      if (opcode === 0x0) {
        if (this.fragmentOpcode !== 0x1) { this.close(1002, "Unexpected continuation"); return; }
        this.fragments.push(payload);
        this.fragmentBytes += payload.length;
        if (this.fragmentBytes > MAX_WS_PAYLOAD) { this.close(1009, "Message too large"); return; }
        if (fin) {
  ...
  private consumeText(payload: any): void {
    try {
      const parsed = JSON.parse(payload.toString("utf8"));
      if (parsed && typeof parsed.type === "string") this.onMessage(parsed as ClientMessage);
    } catch {
...
function serveFile(res: any, file: string): void {
...
const requestHandler = (req: any, res: any): void => {
  ...
  if (file !== base && !file.startsWith(normalizedBase)) { res.writeHead(403); res.end("Forbidden"); return; }
  if (fs.existsSync(file)) serveFile(res, file);
  else serveFile(res, path.join(PUBLIC, "index.html"));
};
...
server.on("upgrade", (req: any, socket: any) => {
  if (req.url !== "/ws" || req.headers.upgrade?.toLowerCase() !== "websocket") { socket.destroy(); return; }
  const key = req.headers["sec-websocket-key"];
  const version = req.headers["sec-websocket-version"];
  if (!key || version !== "13") { socket.destroy(); return; }

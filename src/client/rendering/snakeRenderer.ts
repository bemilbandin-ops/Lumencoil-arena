...
import { clamp, type SnakeSnapshot, type Vec2 } from "../../shared/types.js";
import { darken, skinFor, traceBody, withAlpha } from "./renderMath.js";
import type { VisualEnv } from "./renderTypes.js";

type SnakeSprites = { head: HTMLImageElement; body: HTMLImageElement; tail: HTMLImageElement };

const FIRE_DRAGON = createSprites("/assets/snakes/fire-dragon");

function createSprites(base: string): SnakeSprites | undefined {
...
function ready(sprites: SnakeSprites | undefined): sprites is SnakeSprites {
  return !!sprites && Object.values(sprites).every(image => image.complete && image.naturalWidth > 0);
}

export function drawSnake(env: VisualEnv, s: SnakeSnapshot, isMe: boolean, now: number, spawnT: number, playerLevel: number): boolean {
  if (s.body.length < 2) return false;
  ...
  if (!isMe && (Math.abs(head.x - camera.x) > viewX || Math.abs(head.y - camera.y) > viewY)) return false;
  ...
  if (sprites) {
    ...
    for (let i = 2; i <= s.body.length - 2; i += step) drawSpriteBody(ctx, s.body, i, radius, sprites.body);
    drawSpriteHead(ctx, s, radius, spawnT, sprites.head);
  } else {
    for (let i = 2; i <= s.body.length - 2; i += step) drawBodySegment(ctx, s.body[i]!, i, radius, palette, now);
...
function drawSpriteBody(ctx: CanvasRenderingContext2D, body: Vec2[], index: number, radius: number, image: HTMLImageElement): void {
  const p = body[index]!;
  ...
  ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(angle); ctx.drawImage(image, -size / 2, -size / 2, size, size); ctx.restore();
}

function drawSpriteTail(ctx: CanvasRenderingContext2D, body: Vec2[], radius: number, image: HTMLImageElement): void {
  ...
  ctx.save(); ctx.translate(tail.x, tail.y); ctx.rotate(angle); ctx.drawImage(image, -width, -height / 2, width, height); ctx.restore();
}

function drawSpriteHead(ctx: CanvasRenderingContext2D, s: SnakeSnapshot, radius: number, spawnT: number, image: HTMLImageElement): void {
  const head = s.body[0]!;
  const scale = .72 + spawnT * .28;
  const size = radius * 5.15 * scale;
  ctx.save(); ctx.translate(head.x + Math.cos(s.angle) * radius * .35, head.y + Math.sin(s.angle) * radius * .35); ctx.rotate(s.angle); ctx.drawImage(image, -size * .46, -size / 2, size, size); ctx.restore();
}
...
function drawLabel(ctx: CanvasRenderingContext2D, s: SnakeSnapshot, radius: number, isMe: boolean, zoom: number, playerLevel: number): void {
  ...
  ctx.font = `900 ${levelSize}px system-ui, sans-serif`; ctx.strokeStyle = "rgba(0,0,0,.88)"; ctx.lineWidth = Math.max(2, 3.5 * invZoom); ctx.strokeText(levelText, head.x, y - nameSize * .92); ctx.fillStyle = levelColor; ctx.fillText(levelText, head.x, y - nameSize * .92);

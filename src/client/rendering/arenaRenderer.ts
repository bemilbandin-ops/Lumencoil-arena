import { clamp, type Vec2 } from "../../shared/types.js";
import { mod } from "./renderMath.js";
import type { GroundSpeck, VisualEnv } from "./renderTypes.js";

export function drawBackdrop(env: VisualEnv, width: number, height: number, _now: number, specks: GroundSpeck[]): void {
  ...
  for (let i = 0; i < specks.length; i += 5) {
    ...
    ctx.beginPath(); ctx.ellipse(px, py, rx, ry, (s.x - .5) * .7, 0, Math.PI * 2); ctx.fill();
  }
  ...
  const vignette = ctx.createRadialGradient(
    width * .5, height * .46, Math.min(width, height) * .12,
...
export function drawWorldBoundary(ctx: CanvasRenderingContext2D, arenaRadius: number): void {
  ctx.save();
  ctx.beginPath(); ctx.arc(0, 0, arenaRadius + 48, 0, Math.PI * 2); ctx.strokeStyle = "#07090b"; ctx.lineWidth = 96; ctx.stroke();
  ctx.beginPath(); ctx.arc(0, 0, arenaRadius, 0, Math.PI * 2); ctx.strokeStyle = "#15181b"; ctx.lineWidth = 30; ctx.stroke();
  ctx.beginPath(); ctx.arc(0, 0, arenaRadius - 5, 0, Math.PI * 2); ctx.strokeStyle = "#252a2e"; ctx.lineWidth = 16; ctx.stroke();
  ctx.beginPath(); ctx.arc(0, 0, arenaRadius - 12, 0, Math.PI * 2); ctx.strokeStyle = "#3b4146"; ctx.lineWidth = 5; ctx.stroke();
  ctx.beginPath(); ctx.arc(0, 0, arenaRadius - 15, 0, Math.PI * 2); ctx.strokeStyle = "rgba(205,211,216,.46)"; ctx.lineWidth = 2; ctx.stroke();
  ctx.restore();
}

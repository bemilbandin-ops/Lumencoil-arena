...
import { clamp, normalizeAngle, type SnakeSnapshot, type Vec2 } from "../shared/types.js";

export function previewLocalSnake(snake: SnakeSnapshot, targetAngle: number, boost: boolean): SnakeSnapshot {
  if (!snake.body.length) return snake;
  ...
  const body: Vec2[] = snake.body.map((point, index) => {
